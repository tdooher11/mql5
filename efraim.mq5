#include <trade/trade.mqh>
    ...
    input int dev = 25;
int magic_number = 0;
input int type1period = 20;
input int closeallvalue = 20000;
...
int OnInit() {
    ...
    EURUSDhandle = iMA("EURUSD", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    GBPUSDhandle = iMA("GBPUSD", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    AUDUSDhandle = iMA("AUDUSD", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    EURGBPhandle = iMA("EURGBP", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    EURCHFhandle = iMA("EURCHF", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    EURCADhandle = iMA("EURCAD", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    GBPCHFhandle = iMA("GBPCHF", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    USDCADhandle = iMA("USDCAD", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    USDCHFhandle = iMA("USDCHF", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    AUDJPYhandle = iMA("AUDJPY", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    CHFJPYhandle = iMA("CHFJPY", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    EURJPYhandle = iMA("EURJPY", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    GBPJPYhandle = iMA("GBPJPY", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
    USDJPYhandle = iMA("USDJPY", PERIOD_H4, type1period, 0, MODE_SMA, PRICE_CLOSE);
...
void OnTick() {
    ...
    //if(AccountInfoDouble(ACCOUNT_EQUITY)-AccountInfoDouble(ACCOUNT_BALANCE)>closeallvalue)
    //{
    //   closeall();
    //}
...
void doOrderType1(string symbol) {

    double currentMA = calculateMovingAverage(symbol, type1period);
    MqlTick ma_tick = {
        0
    };
    SymbolInfoTick(symbol, ma_tick);
    double currentPrice = ma_tick.bid;
    string isOrderOpen = orderIsOpen(symbol, 1);
    CTrade trade;
   
    if (isOrderOpen == "false") {
        if (currentPrice > currentMA) {
            openOrder(symbol, 1, ORDER_TYPE_BUY);
        }
        if (currentPrice < currentMA) {
            openOrder(symbol, 1, ORDER_TYPE_SELL);
        }
    }
    if (isOrderOpen == "long") {
        Print("inside order openlong");
        if (currentPrice < currentMA) {
            trade.PositionClose(symbol);
        }
    }
    if (isOrderOpen == "short") {
         Print("inside order openshort");
        if (currentPrice > currentMA) {
            //Print("inside currentprice less than current ma");
            trade.PositionClose(symbol);
...
string orderIsOpen(string symbol, int type) {
    int total = PositionsTotal();
    int count = 0;
    string currentopen = "false";

    for (int cnt = 0; cnt <= total - 1; cnt++) {
        if (PositionSelect(symbol) && PositionGetInteger(POSITION_MAGIC) == type) {
...
void openOrder(string symbol, int type, string shortorlong) {

    if (shortorlong == ORDER_TYPE_BUY) {
    ...
    if (shortorlong == ORDER_TYPE_SELL) {
        MqlTradeRequest request = {
            0
        };
        ZeroMemory(request);
...
double calculateMovingAverage(string symbol, int period) {

    if (symbol == "EURUSD") {
        double EURUSDmovingAverageValues[];
        ArraySetAsSeries(EURUSDmovingAverageValues, true);
        if (CopyBuffer(EURUSDhandle, 0, 0, period, EURUSDmovingAverageValues) < 0) {
    ...
    if (symbol == "GBPUSD") {
        ...
        if (CopyBuffer(GBPUSDhandle, 0, 0, period, GBPUSDmovingAverageValues) < 0) {
            Print("CopyBufferMA2 error =", GetLastError());
        }
    ...
    if (symbol == "AUDUSD") {
        ...
        if (CopyBuffer(AUDUSDhandle, 0, 0, period, AUDUSDmovingAverageValues) < 0) {
    ...
    if (symbol == "EURGBP") {
        ...
        if (CopyBuffer(EURGBPhandle, 0, 0, period, EURGBPmovingAverageValues) < 0) {
            Print("CopyBufferMA1 error =", GetLastError());
        }
    ...
    if (symbol == "EURCHF") {
        ...
        if (CopyBuffer(EURCHFhandle, 0, 0, period, EURCHFmovingAverageValues) < 0) {
            Print("CopyBufferMA2 error =", GetLastError());
        }
    ...
    if (symbol == "EURCAD") {
        ...
        if (CopyBuffer(EURCADhandle, 0, 0, period, EURCADmovingAverageValues) < 0) {
            Print("CopyBufferSAR error =", GetLastError());
        }
    ...
    if (symbol == "GBPCHF") {
        ...
        if (CopyBuffer(GBPCHFhandle, 0, 0, period, GBPCHFmovingAverageValues) < 0) {
            Print("CopyBufferMA1 error =", GetLastError());
        }
    ...
    if (symbol == "USDCAD") {
        ...
        if (CopyBuffer(USDCADhandle, 0, 0, period, USDCADmovingAverageValues) < 0) {
            Print("CopyBufferMA2 error =", GetLastError());
        }
    ...
    if (symbol == "USDCHF") {
        ...
        if (CopyBuffer(USDCHFhandle, 0, 0, period, USDCHFmovingAverageValues) < 0) {
            Print("CopyBufferSAR error =", GetLastError());
        }
    ...
    if (symbol == "AUDJPY") {
        ...
        if (CopyBuffer(AUDJPYhandle, 0, 0, period, AUDJPYmovingAverageValues) < 0) {
            Print("CopyBufferMA1 error =", GetLastError());
        }
    ...
    if (symbol == "CHFJPY") {
        ...
        if (CopyBuffer(CHFJPYhandle, 0, 0, period, CHFJPYmovingAverageValues) < 0) {
            Print("CopyBufferMA2 error =", GetLastError());
        }
    ...
    if (symbol == "EURJPY") {
        ...
        if (CopyBuffer(EURJPYhandle, 0, 0, period, EURJPYmovingAverageValues) < 0) {
            Print("CopyBufferSAR error =", GetLastError());
        }
    ...
    if (symbol == "GBPJPY") {
        ...
        if (CopyBuffer(GBPJPYhandle, 0, 0, period, GBPJPYmovingAverageValues) < 0) {
            Print("CopyBufferMA1 error =", GetLastError());
        }
    ...
    if (symbol == "USDJPY") {
        ...
        if (CopyBuffer(USDJPYhandle, 0, 0, period, USDJPYmovingAverageValues) < 0) {
            Print("CopyBufferMA1 error =", GetLastError());
        }
